		...
		if(msg.length>0) {
			$(msg).each(function(i) {
				...
				if(i==0) {
					...
					_html += '<dl><dt><a href="javascript:;">'+this.list[0].title+'</a></dt><dd>'+this.list[0].summary+'……<a href="'+basePath+'pregnantart/pregnantart_findPregnantArticleDetail.action?id='+this.list[0].id+'">[详细]</a></dd></dl>';
					_html += '<ul>';
					...
					_html += '<a href="'+basePath+'pregnantart/pregnantart_findPregnantArticleList.action?id='+this.map.id+'" class="by-more">[更多]</a>';
					_html += '<ul>';
					$(this.list).each(function(n) {
						_html += '<li><a href="'+basePath+'pregnantart/pregnantart_findPregnantArticleDetail.action?id='+this.id+'">'+this.title+'</a></li>';
					});
