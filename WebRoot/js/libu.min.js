var libu = {
	/** 孕妇分类 **/
	pregnantCategory: function() {
		libu.post({url:basePath+"pregnanttype/pregnanttype_findCategory.action"}, libu.pregnantCategoryCallback);
	},
	/** 孕妇分类回调函数 **/
	pregnantCategoryCallback: function(msg) {
		console.log(msg);
	},
	pregnantArticleCategoryList: function() {
		libu.post({url:basePath+"/pregnantart/pregnantart_findArticleCategoryList.action", data: {size:8}}, libu.pregnantArticleCategoryListCallback);
	},
	pregnantArticleCategoryListCallback: function(msg) {
		console.log(msg);
	},
	
	/** ajax的post公共调用方法 **/
	post: function(option, callback) {
		$.ajax({
			type:"POST",
			async:(!!option.async)?false:true,
			url:option.url,
			data:option.data,
			success: function(msg) {
				!!callback==true?callback(msg):"";
			}
		})
	}
};
/** 孕妇健康相关操作初始化调用 **/
var pregnantType = {
	init: function() {
		libu.pregnantCategory();
		libu.pregnantArticleCategoryList();
	}
}