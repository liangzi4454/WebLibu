		...
		if(msg.length>0) {
			$(msg).each(function(i) {
				...
				if(i==0) {
					...
					_html += '<dl><dt><a href="javascript:;">'+this.list[0].title+'</a></dt><dd>'+this.list[0].summary+'……<a href="javascript:;">[详细]</a></dd></dl>';
					_html += '<ul>';
					$(this.list).each(function(n) {
						if(n<5) {
							_html += '<li><a href="javascript:;">'+this.title+'</a></li>';
						}
					});
					_html += '</ul>';
					$("#style_0").html(_html);
					...
					_html += '<a href="javascript:;" class="by-more">[更多]</a>';
					_html += '<ul>';
					$(this.list).each(function(n) {
						_html += '<li><a href="javascript:;">'+this.title+'</a></li>';
					});
